...
pub fn get_extended_path() -> String {
    ...
    paths.extend(get_managed_path_entries());
...
fn get_managed_path_entries() -> Vec<String> {
    let runtime_dir = platform::get_manager_runtime_dir();

    if platform::is_windows() {
        vec![
            format!("{}\\nodejs", runtime_dir),
            format!("{}\\git_env\\cmd", runtime_dir),
            format!("{}\\openclaw_app\\node_modules\\.bin", runtime_dir),
        ]
    } else {
        vec![
...
pub fn get_managed_node_path() -> Option<String> {
    let runtime_dir = platform::get_manager_runtime_dir();
...
pub fn get_managed_npm_path() -> Option<String> {
    let runtime_dir = platform::get_manager_runtime_dir();
    let candidate = if platform::is_windows() {
        format!("{}\\nodejs\\npm.cmd", runtime_dir)
    } else {
...
pub fn get_managed_git_path() -> Option<String> {
    if !platform::is_windows() {
        return None;
    }
    ...
    let candidate = format!(
        "{}\\git_env\\cmd\\git.exe",
        platform::get_manager_runtime_dir()
    );
    Path::new(&candidate).exists().then_some(candidate)
}

pub fn get_managed_openclaw_path() -> Option<String> {
    let runtime_dir = platform::get_manager_runtime_dir();
    let candidate = if platform::is_windows() {
        format!(
        ...
        format!("{}/openclaw_app/node_modules/.bin/openclaw", runtime_dir)
    };
...
pub fn get_npm_path() -> Option<String> {
    if let Some(path) = get_managed_npm_path() {
        return Some(path);
    }
...
pub fn get_openclaw_path() -> Option<String> {
    if let Some(path) = get_managed_openclaw_path() {
        info!("[Shell] 在托管运行时中找到 openclaw: {}", path);
        return Some(path);
    }
...
pub fn run_openclaw(args: &[&str]) -> Result<String, String> {
    ...
    let openclaw_path = get_openclaw_path().ok_or_else(|| {
        ...
        "找不到 openclaw 命令，请先在应用内完成 OpenClaw 安装".to_string()
    })?;
...
pub fn spawn_openclaw_gateway() -> io::Result<()> {
    ...
    let openclaw_path = get_openclaw_path().ok_or_else(|| {
        ...
        io::Error::new(
            ...
            "找不到 openclaw 命令，请先在应用内完成 OpenClaw 安装",
        )
    })?;
