...
pub async fn run_doctor() -> Result<Vec<DiagnosticResult>, String> {
    ...
    results.push(DiagnosticResult {
        ...
        suggestion: if openclaw_installed {
            ...
            Some("请先在应用内完成 OpenClaw 安装；如需手动安装，可运行: npm install -g openclaw@latest --registry https://registry.npmmirror.com".to_string())
        },
    });

    // 检查 Node.js
    let node_check = if let Some(path) = shell::get_managed_node_path() {
        shell::run_command_output(&path, &["--version"])
    } else {
...
pub async fn get_system_info() -> Result<SystemInfo, String> {
    ...
    let node_version = if let Some(path) = shell::get_managed_node_path() {
